...
$(function(){
    ...
    //导航设置
    var Route = "/o2o/route/"+wxGLOBAL.shopSid+"/tools-qrcode-qrcode_pay_focus";
    $("#navigation_set").attr("href",Route);

    //导航设置
    var wifiRoutes = "/o2o/route/"+wxGLOBAL.shopSid+"/tools-wifi-shop_wifi_snapshoot";
    $("#wifiRouter").attr("href",wifiRoutes);
	...
	var isBuy = false ;//是否购买专属公众号 套餐 没有则不显示
   /***********************初始化数据 end*****************************************/
...
/*******************绑定方法 end*******************************************/
    ...
    function checkisBuySec(){
        //判断是否购买套餐
        var _this =this;
        $.ajax({
            url:requestUrl.Payment.selectIsPayTc,
            data:{shop_sid:wxGLOBAL.shopSid,type:'beautify_mp'},
            type:"get",
            dataType: "json",
            global:false,
            success :function(data){
                if(data.status==200 ){
                    ...
                    if(resultList!=null && resultList.length>0) {
                                isBuy = true;//已购买
                                getSelectCondition(curPage);//查询列表
                    }else{
                        //没有购买
                        isBuy = false;
                        $("#tempUserInfo").html(' <p class="lh40 text-center" id="yesNull">您还未开通专属公众号，暂时无法使用此功能</p>');
                    }
    ...
    function selectUserListFun(page,cell_phone,start_time,end_time,bind_status){
      ...
      var params = {
         ...
         is_bind : bind_status||'',
          limit : rows, //显示行
          offset : offest //从第几行开始
       };
    
    //获取数据
    $.get(requestUrl.user.fansListUser,params,function(resData){
        ...
        ///      datas = JSON.parse(datas);//转json
           ...
           if (resData.list && resData.list.length>0) {
                          ...
                          for (var i = 0; i < resData.list.length; i++) {
                          	 if (resData.list[i].cell_phone) {
                          	 	 var tel = resData.list[i].cell_phone;
                                 var m = tel.substring(3, tel.length-4).replace(/./g, "*");
                                 var bindedPhone = tel.substr(0,3) + m + tel.substr(tel.length-4);
                                 resData.list[i].cell_phones = bindedPhone;
                          	 }


                              //时间转化
                              var hmDate = resData.list[i].subscribe_time ; // 关注日期
                              if (hmDate) {
                                  var newDate = new Date(parseInt(hmDate));
                                  resData.list[i].subscribe_times = newDate.format('yyyy-MM-dd');
                              }else{
                                  resData.list[i].subscribe_times = "--";
                              }

                              //状态为空的情况 自定义赋值 5
                              var bind_status = resData.list[i].bind_status ;
                              if (!bind_status) {
                                  resData.list[i].bind_status = 5 ;//潜在
                              }

                          	
                          }
                
                            laytpl(insertHtml).render(resData.list,function(html){
                            ...
                            laypage({
                                ...
                                jump: function(obj,first){
                                  ...
                                  if(!first){
                                      var bindStatus = $("#bindStatus").val();
                                      selectUserListFun(obj.curr,cell_phone,start_time,end_time,bindStatus);
                                  }
                                }
                            });
                          }); 

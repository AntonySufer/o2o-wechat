(function(){
    ...
    var payActionServer =wxGLOBAL.payActionServer;//支付服务
	var sellActionServer =wxGLOBAL.sellActionServer;//支付服务
	...
	var requestUrl={
		...
		user:{
			...
			fansListUser : wxServer+"/mp/user/fans/list",//粉丝管理
		},
		shop:{
		...
		//购买套餐
		Payment:{
			  selectShopInfo:saasServer+"/saas.shop/shop/{{shop_id}}",//查询店铺信息
			  selectIsPayTc:wxServer+"/auths/validate/shop",//查询是否开通
              selectTcList : sellActionServer+"/sell.fee/commodity", //查询套餐列表
			  getOrderIndex : sellActionServer+"/sell.fee/shop/{{shop_id}}/order/id",//获取订单索引
			  placeOrder: sellActionServer+"/sell.fee/shop/{{shop_id}}/order",//下单 新建
			  aleryOrder: sellActionServer+ '/sell.fee/shop/{shop_sid}/order/{oder_id}/pay?pay_way={pay_way}',//已有订单获取支付签名,//下单 已有订单获取签名
			  orderPay : payActionServer+"unify/qrpay", //套餐支付
			  selectBeautMoney:payActionServer+"acc/remaining.pc"//查询美丽宝余额
	     }
	};
